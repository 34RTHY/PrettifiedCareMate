...
class Chatapp:
    ...
    def generate_medical_code_report(self,Suggested_ICD_11_Codes_Text, Suggested_CPT_Codes_Text, Suggested_HCPCS_Codes_Text, output_path):
        page_height = caremate.writer.calculate_height([Suggested_ICD_11_Codes_Text,Suggested_CPT_Codes_Text,Suggested_HCPCS_Codes_Text])+300
    ...
    def generate_patient_mode_report(self,Possible_diseases, Treatments_for_each_disease,doctor_or_pharmacy,Next_steps_for_the_patient, output_path):
        page_height = self.calculate_height([Possible_diseases,Treatments_for_each_disease,doctor_or_pharmacy,Next_steps_for_the_patient])+300
        wxh_page = (612.0,page_height)
        y_offset = page_height
        c = canvas.Canvas(output_path, pagesize=wxh_page)
    ...
    def generate_doctor_mode_report(self,Suggested_Diagnosis, Suggested_Actions,Suggested_Laboratory_Tests,output_path):
        page_height = self.calculate_height([Suggested_Diagnosis,Suggested_Actions,Suggested_Laboratory_Tests])+300
        wxh_page = (612.0,page_height)
        y_offset = page_height
        c = canvas.Canvas(output_path, pagesize=wxh_page)
...
"""
                ...
                self.generate_patient_mode_report(Possible_diseases, Treatments_for_each_disease,doctor_or_pharmacy,Next_steps_for_the_patient,f'./pdfout/{File_stamp}.pdf')
                with open(f"./pdfout/{File_stamp}.pdf", "rb") as pdf_file:
                    PDFbyte = pdf_file.read()
...
"""
                ...
                timestamp = ''.join(c if c not in invalid_chars else '_' for c in generated_time)
                File_stamp = f"{timestamp}"
                self.generate_doctor_mode_report(Suggested_Diagnosis, Suggested_Actions,Suggested_Laboratory_Tests,f'./pdfout/{File_stamp}.pdf')
                with open(f"./pdfout/{File_stamp}.pdf", "rb") as pdf_file:
                    PDFbyte = pdf_file.read()
